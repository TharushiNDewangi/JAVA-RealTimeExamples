...
select m.MeetingID,r.RoomID,m.MeetingTitle
...
--5. Find the first name and last name of employees who participated in Appraisal meeting on 18th August 2016. 
 ...
 from Employee_Details e,Meeting_Details m,Meeting_Employees me
 where me.MeetingID=m.MeetingID and e.EmployeeID=me.EmployeeID 
and m.MeetingDate='18-Aug-2016'
 and m.MeetingTitle ='Appraisal Meeting'

 --6. Display the highest and the lowest capacity of a room
 select max(RoomSize) as higherscap,min(RoomSize) as mincap
 From Room_Details

 --7. Find the number of employees from India
 select count(EmployeeID) as numinindia
 from Employee_Details
 where country ='india'

 --8. Find the number of employees who had participated in Appraisal Meeting_Details in August. 
 select count(e.EmployeeID) as emp
 from Employee_Details e,Meeting_Details m,Meeting_Employees me
 where me.MeetingID=m.MeetingID and e.EmployeeID=me.EmployeeID 
  and m.MeetingTitle ='Appraisal Meeting'and datepart(month,m.MeetingDate)=8

  --9. Find the number of employees from each country
  select count(EmployeeID) as countincontry,country
  from Employee_Details
  group by Country
  
  --10. For each type of Meetings display the number of employees who had participated
  select  m.MeetingTitle,count(e.EmployeeID) as numofemployee
 from Employee_Details e,Meeting_Details m,Meeting_Employees me
 where me.MeetingID=m.MeetingID and e.EmployeeID=me.EmployeeID
...
select country ,count(EmployeeID) as mostnumofemp
from Employee_Details
group by country
...
--this is wrong
select r.RoomName,m.MeetingDate
from Meeting_Details m
left outer join Room_Details r
on m.RoomID=r.RoomID
where not datepart(month,m.MeetingDate)=8 
--rightone
select RoomName,m.MeetingDate
from Room_Details r,Meeting_Details m
where r.RoomID not in (select RoomID from Meeting_Details m where  datepart(month,m.MeetingDate)=8 )
